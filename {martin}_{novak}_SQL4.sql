WITH t_year_0 AS (
		...
		GROUP BY monthly_salary0 , industry0 ,year0 
),
t_year_1 AS (
		SELECT
		...
		WHERE industry = '_All_in_one'
		GROUP BY monthly_salary1 , industry1 ,year1 
)
SELECT
	...
	round((((price0 - price1)/price1)*100) - (((monthly_salary0 - monthly_salary1)/monthly_salary1)*100),2) AS difference 
FROM t_year_0 t0
